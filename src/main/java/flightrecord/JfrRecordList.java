...
public final class JfrRecordList {
  ...
  void main() throws IOException, ParseException {
    ...
    try (Recording recording = new Recording(configuration)) {
      ...
      recording.stop();
      recording.dump(Path.of("test-identity.jfr"));
    }
    ...
    try (Recording recording = new Recording(configuration)) {
      ...
      List<value.weather.WeatherComputation.WeatherData> values = new ArrayList<>(iterations);
      for (int i = 0; i < iterations; i++) {
      ...
      recording.stop();
      recording.dump(Path.of("test-value.jfr"));
    }
  }
}
